...
> "플러그인 자동 업데이트를 켜면 새 버전이 나올 때 자동으로 반영됩니다. `/plugin` 입력 → Marketplaces 탭 → neosapience-douzonebot-plugin → Enable auto-update를 클릭해주세요. 한 번만 설정하면 됩니다."
...
3. **영수증 폴더** (전체 모드만): 영수증 이미지 폴더 경로 → 이미지 파일 개수 확인
