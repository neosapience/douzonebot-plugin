...
**OCR 결과 사전 준비 (선택):** 영수증 옆에 같은 이름의 `.ocr.md` 파일을 두면 Vision AI를 건너뛰고 해당 텍스트를 바로 사용합니다. 예: `receipt_0305.jpg` → `receipt_0305.ocr.md` (PDF도 동일: `invoice.pdf` → `invoice.pdf.ocr.md`).

`.ocr.md` 권장 형식 (특히 배민/PG 거래는 **실공급자 상호 + 사업자등록번호 필수**):
...
> PG/배달앱은 카드전표의 대행사(우아한형제들/KCP 등)가 아니라 **실제 음식점**의 상호·사업자번호를
> 적습니다. 사업자번호가 빠지면 review에서 `⚠️ 실공급자 사업자등록번호 누락` 경고가 표시됩니다.
> 자세한 규약은 `go` 스킬의 "사전 OCR 작성 규약" 참고.

### 알아두면 좋은 경비 규정

에이전트가 알아서 사용자에게 안내해야 하는 규정입니다:

**실공급자 정보 필수 거래처 — 영수증 첨부 & 실공급자상호/사업자번호 입력 필수**

다음 거래처들은 카드매출표상 표기와 실제 공급자가 달라, 반드시 실물 영수증을
첨부하고 **실공급자상호 + 실공급자 사업자등록번호**를 입력해야 합니다:

- **배민/배달앱**: 우아한형제들, 배달의민족, 쿠팡이츠, 요기요
- **백화점/쇼핑몰**: 현대백화점 (무역센터점 등 전지점), 신세계프라퍼티(코엑스몰)
- **PG사**: 나이스정보통신(주), 엔에이치엔한국사이버결제(NHN KCP),
  (주)케이지이니시스, 카카오페이, 네이버페이, 토스

Post-verify가 이들 거래처에서 영수증/실공급자 정보가 누락되면 자동으로
감지하고 마지막에 수정을 안내합니다. 영수증이 없으면 비고란에 사유가
자동 기입됩니다.

**기타 대형 쇼핑몰 (스타필드, 롯데백화점, 신세계백화점, 이마트 등)**
